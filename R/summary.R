summary.dpm<-function(object,...){
    ...
    ans$S<-rbind(object$predtime[index], object$Spred[index],object$Spredu[index],object$Spredl[index])
    }else{
    index<-c(11,21,31,41)
    ans$S<-rbind(object$predtime[index], object$Spred[index],object$Spredu[index],object$Spredl[index])
    }
    rownames(ans$S)<-c("Time","Estimated Survival","Upper Credible Interval", "Lower Credible Interval")
    colnames(ans$S)<-c("1/4 Observation Time","1/2 Observation Time","3/4 Observation Time","Observation Time")
...
summary.ddp<-function(object,...){
    ...
    if(object$usertime){
	    ...
	    ans$loghr[[i]]<-rbind(object$predtime[index],object$loghr.est[i,index], object$loghru[i,index],object$loghrl[i,index])
	    rownames(ans$loghr[[i]])<-c("Time","Estimated Log Hazard Ratio","Upper Credible Interval", "Lower Credible Interval")
	    colnames(ans$loghr[[i]])<-c("1/4 Observation Time","1/2 Observation Time","3/4 Observation Time","Observation Time")
	    }
   names(ans$loghr)<-names(object$xmean)
