...

{
  ...
  fclose(fp);*/
	...
	iteration ++;
  } while ((msamp < nsamp)&(iteration<100));
  ...
  /*fp = fopen("file", "w");
  fprintf(fp,"problem is with invert!\n");
  fclose(fp);*/
  for (i=0; i<ncent; i++){
...

{
  ...
  /*if ((p->x < xl) || (p->x > xr)) {
  fp = fopen("file", "w");
  fprintf(fp,"exit 1 inside invert!\n");
  fclose(fp);
  exit(1);
  }*/
  if(p->x<=xl){
...

{
 ...
 /* fp = fopen("file", "w");
  ...
  cumulate(env);
     /*fp = fopen("file", "w");
     fprintf(fp,"nothing wrong with update\n");
	 fclose(fp);*/
  return 0;
}
...
{
   return unif_rand();
   //return ((double)R::runif(0,1) + 0.5)/((double)RAND_MAX + 1.0);
}
